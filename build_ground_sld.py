...
class Sld:
    ...
    def __init__(self, query, db, positives, negatives, mode):
        ...
        nx.set_node_attributes(digraph, 'goals', {root_node_id: [query]})
        # nodes should contain goal: list of subgoals
        nx.set_node_attributes(digraph, 'substitution', {root_node_id: {}})
    ...
    def step(self):
        ...
        if node_goals:
            ...
            tokens = [
                subgoal.relation.name
            ]  #+ [ x.name for x in subgoal.arguments if x.name[0].islower() ]
            ...
            for rule_idx, rule in enumerate(rel_db_rules):
                ...
                if mgu is not None:
                    ...
                    rest_subgoals = unification.apply_substitution(
                        rest_subgoals, mgu)
                    if len(feature) == 0:
                        ...
                        tokens = [feature[0].relation.name] + [
                            x.name for x in feature[0].arguments
                            if x.name[0].islower()
                        ]
                        ...
                        for rule_f in rel_rules:
                            if rule_f[1] == [] and rule_f[2] == []:
                                ...
                                if subs is not None:
                                    ground_feature = unification.apply_substitution(
                                        feature, subs)[0]
                        ...
                        tokens = [feature[1].relation.name] + [
                            x.name for x in feature[1].arguments
                            if x.name[0].islower()
                        ]
                        rel_rules = db.get_rules_with_tokens(tokens)
                        for rule_f in rel_rules:
                            if rule_f[1] == [] and rule_f[2] == []:
                                feature_copy = deepcopy(feature)
                                subs = unification.unify(
                                    rule_f[0], feature_copy[1])
                                if subs is not None:
                                    ground_feature = unification.apply_substitution(
                                        feature_copy, subs)[0]
                                    ground_features.append(ground_feature)
                    ...
                    nx.set_node_attributes(self.graph, 'goals', {
                    ...
                    previous_subst = nx.get_node_attributes(
                        self.graph, 'substitution')[node_id]
                    new_node_subst = unification.compose_mgus(
                        previous_subst, mgu)
                    nx.set_node_attributes(self.graph, 'substitution',
                                           {new_node_id: new_node_subst})

                    if not new_node_goals:

                        for positive in self.positives:
                            is_positive = all((k in new_node_subst and
                                               new_node_subst[k] == v)
                                              for k, v in positive.iteritems())
                            if is_positive:
                        ...
                        for negative in self.negatives:
                            is_negative = all((k in new_node_subst and
                                               new_node_subst[k] == v)
                        ...
                        self.graph.add_edge(
                            new_node_id, new_node_id, mgu={}, feat=[idx])
    ...
    def add_back_edges(self):
        ...
        for node_id in self.graph.nodes():
            ...
            self.graph.add_edge(node_id, query_node, mgu={}, feat=[idx])
        return None
    ...
    def dump(self):
        ...
        for edge in self.graph.edges(data=True):
            ...
            edge_mark = edge_mark + ':' + ','.join([
                '@'.join([str(x), str(y)])
                for x, y in zip(feats, ['1.0'] * len(feats))
            ])
            edge_marks.append(edge_mark)

        edge_marks_str = '\t'.join(edge_marks)
        line = '\t'.join([
            query, query_node, pos_nodes, neg_nodes, nodes_count, edges_count,
            label_dependency_count, edge_marks_str
        ])
    ...
    for set_name in set_names:
        ...
        with open(examples_path) as f:
            for idx, ppr_example in enumerate(f.readlines()):
                ...
                positive_examples = [
                    example[1:] for example in array[1:]
                    if example.strip()[0] == '+'
                ]
                negative_examples = [
                    example[1:] for example in array[1:]
                ...
                for positive_example in positive_examples:
                    positive_substitution = unification.unify(
                        Atom(query), Atom(positive_example))
                    positive_substitutions.append(positive_substitution)

                negative_substitutions = []
                for negative_example in negative_examples:
                    negative_substitution = unification.unify(
                        Atom(query), Atom(negative_example))
                    negative_substitutions.append(negative_substitution)

                sld_graph = Sld(
                    Atom(query), db, positive_substitutions,
                    negative_substitutions, set_name)
