...
def get_proof_graph(ppr_grounded_line, feature_vector_size):
	...
	for edge in edges:
		...
		source = int(source)
		target = int(target)
		feature_weights = [ feature.split('@') for feature in feature_weights.split(',')]
		feature_weights = [ (int(feature_weight[0]), float(feature_weight[1])) for feature_weight in feature_weights]
...
def get_proof_graph_tensor( proof_graph, feature_vector_size ):
	...
	size = len(node_list)
	featured_adjacency_matrix = np.array([[ [0.0]*feature_vector_size for x in range(size)] for y in range(size)])
	...
	featured_adjacency_matrix = np.reshape( featured_adjacency_matrix, [size, size, -1] )
	...
	return [ one_hot_query_vector, featured_adjacency_matrix, correct_answer_vector, incorrect_answer_vector]
...
def dump_graph_tensor(idx, tensors_dir, ppr_grounded_line):
	...
	sparse_tensor_path = os.path.join( tensors_dir, '{}-sparse.pickle'.format(idx))
...
for set_name in set_names:
	...
	sld_grounded_path  = os.path.join(processed_data_dir, program_name + '-{}.grounded'.format(set_name) )
