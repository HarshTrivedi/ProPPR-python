...
def is_case(substitutions, pick_index):
    ...
    rest_substitutions = substitutions[:pick_index] + substitutions[pick_index +
                                                                    1:]
    ...
    rest_count = len(
        [1 for items in rest_substitutions_items if arg_left.name in items])

    if isinstance(arg_left, Atom) and isinstance(arg_right, Atom):
        if arg_left.relation.name == arg_right.relation.name and len(
                arg_left) == len(arg_right):
            return 1
        else:
            return 2
    if isinstance(arg_left, Constant) and isinstance(
            arg_right, Constant) and arg_left.name != arg_right.name:
        return 2
    elif arg_left.name == arg_right.name:
        return 3
    elif not isinstance(arg_left, Variable) and isinstance(arg_right, Variable):
        return 4
    elif isinstance(arg_right, Atom) and arg_left.name in map(
            lambda x: x.name, arg_right.arguments):
        return 5
...
def unify(x, y):
    ...
    while True:
        ...
        if substitutions is None:
            ...
            pick_cases = list(
                set([
                    is_case(substitutions, pick_index)
                    for pick_index in range(possible_picks)
                ]))
