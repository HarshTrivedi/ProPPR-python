...
featured_adjacency_matrix = tf.placeholder( tf.float32, [ None, None, edge_feature_vector_size ] )
...
def aggregate_by_log_sum( answer_vector, pi ):
    ...
    aggregation = tf.reduce_sum(  tf.log(tf.multiply( answer_vector , pi ) + smoothing ) )
    return aggregation

def get_transition_matrix():

    featured_flattened_adjacency_matrix = tf.reshape( featured_adjacency_matrix, [-1, edge_feature_vector_size] )
    flattened_adjacency_matrix = tf.matmul( featured_flattened_adjacency_matrix, feature_weights )
    nodes_count = tf.shape(one_hot_query_vector)[0]
    transition_matrix = tf.reshape( flattened_adjacency_matrix, [ nodes_count, nodes_count ] )
    ...
    transition_matrix = tf.multiply( tf.nn.relu(transition_matrix), transition_mask )
    ######

    sum_reduced = tf.reduce_sum( transition_matrix, axis = 1, keep_dims = True)
    condition = tf.greater( sum_reduced, tf.zeros_like(sum_reduced, dtype = tf.float32) )
    safe_sum_reduced = tf.where( condition , sum_reduced, (tf.constant(np.inf, dtype = tf.float32))*tf.ones_like(sum_reduced)  )
...
def compute_loss(pi):    
    loss = aggregate_by_log_sum( incorrect_answer_vector, pi ) - aggregate_by_log_sum( correct_answer_vector, pi )
    return loss
...
def get_mrr(correct_scores, incorrect_scores):
    ...
    retrieval = [ (e, -1.0) for e in incorrect_scores] + [ (e, 1.0) for e in correct_scores]
    retrieval.sort(key=(lambda x: x[0]), reverse=True)
    mean_reciprocal_rank = np.mean([ 1/float(i+1) for i, e in enumerate(retrieval) if e[1] == 1.0 ])
    return mean_reciprocal_rank

def get_correct_incorrect_scores(np_pi, np_correct_answer_vector, np_incorrect_answer_vector):
    ...
    correct_scores   = [ a for a,b in zip(np_pi, np_correct_answer_vector) if b > 0]
    incorrect_scores = [ a for a,b in zip(np_pi, np_incorrect_answer_vector) if b > 0]
...
with tf.Session() as sess:
    ...
    tensor_idxs = [ int(os.path.basename(name).split('-')[0]) for name in tensor_file_names]
    tensor_idxs.sort()

    for idx in tensor_idxs:

        with open( os.path.join(tensors_dir, '{}-sparse.pickle'.format(idx)) , 'rb') as f:
            sparse_data = pickle.load(f)
            tensor = [ item.todense() for item in sparse_data]
            feed_dict[idx] = {  one_hot_query_vector:      tensor[0],
    ...
    if run == 'Predict':
        ...
        mrrs = []; aucs = []
        for idx in tensor_idxs:
            train_loss, np_pi, np_correct_answer_vector, np_incorrect_answer_vector = sess.run( [instance_loss, pi, correct_answer_vector, incorrect_answer_vector ], feed_dict = feed_dict[idx] )
            np_feature_weights = sess.run(feature_weights)
            correct_scores, incorrect_scores = get_correct_incorrect_scores(np_pi, np_correct_answer_vector, np_incorrect_answer_vector)
